...
export default function WatchHistory() {
  const { getHistory , deleteHistory , deleteAllHistory , pauseHistory } = useUser();
  ...
  return (
    <div className="w-full flex md:flex-row flex-col h-[calc(100vh-72px)] bg-black overflow-auto text-white lg:gap-4">
      ...
      {/* MAIN BAR */}
      <div className="  w-full h-[calc(100vh-72px)] p-3 md:p-6 ">
        {/* UPPER PART */}
        ...
        <div className="flex pb-20 sm:flex-row flex-col-reverse sm:gap-0 gap-14  md:justify-between w-full">
          {/* History List */}
          {
            (history?.message.length === 0) && (
              <p>No History Yet</p>
            )
          }
          <div className="space-y-4">
            {history?.message?.map((item) => (
              <div key={item._id} className="flex sm:flex-row flex-col w-full  cursor-pointer gap-4 group">
                {/* Thumbnail */}
                <div 
